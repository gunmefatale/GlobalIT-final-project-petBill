...
<head>
    ...
    <script type="text/javascript">
         function ajaxCompare() {
              ...
              $.ajax({ // 설정하는 부분
                 url: 'http://localhost:8080/petBill/hospital/ajaxCompareA.pet', 
                 method:'get', 
    ...
    <script>
		function setHospitalA(hosName, hosNo){
			//opener : 지금 페이지를 오픈해준 페이지(hosCompare.pet)의 태그값 변경해줌.
			opener.document.getElementById("aHospital").value = hosName;
			opener.document.getElementById("aHospitalNo").value = hosNo;
			self.close();
		}
	</script>
			...
			<table class="table table-sm table-bordered ">
				...
				<c:forEach var="hos" items="${hosList}">
					<tbody>
						<tr>
							<td align="center">
								<button value="${hos.hosName}" onclick="setHospitalA('${hos.hosName}','${hos.hosNo}')" class="btn btn-outline-dark btn-sm"> 선택 </button>
							</td>
							<td>${hos.hosName} </td>
