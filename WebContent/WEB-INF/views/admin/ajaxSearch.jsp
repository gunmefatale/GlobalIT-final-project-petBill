...
<head>
    ...
    <script type="text/javascript">
         function ajaxSearch() {
              ...
              $.ajax({ // 설정하는 부분
                 url: 'http://localhost:8080/petBill/admin/ajaxSearch.pet', 
                 method:'get', 
    ...
    <script>
		function setHospital(hosName, hosNo){
			//opener : 지금 페이지를 오픈해준 페이지(hosCompare.pet)의 태그값 변경해줌.
			opener.document.getElementById("hosName").value = hosName;
			opener.document.getElementById("hosNo").value = hosNo;
			self.close();
		}
	</script>
...
<body>
	...
	<!-- ajax 영역 -->
	<%-- 병원 시/도 및 시/구 선택 --%>
		<div id="ajaxSearch" class="divtest" align="center">
			<table class="table">
				<thead>
					<tr>
						<td>
						...
						<label for="addressKindD">시/구 선택</label>
							...
							<button onclick="ajaxSearch()" class="btn btn-secondary btn-sm">지역으로 검색</button>
						</td>
						<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
						<td>
						<%-- 병원명으로 검색 --%>
							<form action="ajaxSearch.pet">
			...
			<table class="table table-sm table-bordered ">
				...
				<c:forEach var="hos" items="${hosList}">
					<tbody>
						<tr>
							<td align="center">
								<button value="${hos.hosName}" onclick="setHospital('${hos.hosName}','${hos.hosNo}')"  class="btn btn-outline-dark btn-sm"> 선택 </button>
							</td>
							<td>${hos.hosName} </td>
		...
		<c:if test="${count > 0}">
			...
			<c:if test="${sidoSel == null && siguSel == null && search == null}">
				<c:if test="${startPage > pageBlock}">
					<a href="/petBill/admin/ajaxSearch.pet?pageNum=${startPage - pageBlock}" class="page-link"> &lt;</a>
				</c:if>
				<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
					<a href="/petBill/admin/ajaxSearch.pet?pageNum=${i}" class="page-link"> ${i} </a>
				</c:forEach>
				<c:if test="${endPage < pageCount}">
					<a href="/petBill/admin/ajaxSearch.pet?pageNum=${startPage + pageBlock}" class="page-link"> &gt; </a>
				</c:if>
			...
			<c:if test="${search != null}">
				<%-- 왼쪽 꺽쇄 '<'  --%>
				<c:if test="${startPage > pageBlock}">
					<a href="/petBill/admin/ajaxSearch.pet?pageNum=${startPage - pageBlock}&search=${search}" class="page-link"> &lt;</a>
				</c:if>
				<%-- 페이지 하단번호 뿌리기 ex < 1 2 3 4 5 > --%>
				<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
					<a href="/petBill/admin/ajaxSearch.pet?pageNum=${i}&search=${search}" class="page-link"> ${i} </a>
				</c:forEach>
				<%-- 오른쪽 꺽쇄 '>'  --%>
				<c:if test="${endPage < pageCount}">
					<a href="/petBill/admin/ajaxSearch.pet?pageNum=${startPage + pageBlock}&search=${search}" class="page-link"> &gt; </a>
			...
			<c:if test="${sidoSel != null && siguSel != null}">
				<%-- 왼쪽 꺽쇄 '<'  --%>
				<c:if test="${startPage > pageBlock}">
					<a href="/petBill/admin/ajaxSearch.pet?pageNum=${startPage - pageBlock}&sidoSel=${sidoSel}&siguSel=${siguSel}" class="page-link"> &lt; </a>
				</c:if>
				<%-- 페이지 하단번호 뿌리기 ex < 1 2 3 4 5 > --%>
				<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
					<a href="/petBill/admin/ajaxSearch.pet?pageNum=${i}&sidoSel=${sidoSel}&siguSel=${siguSel}" class="page-link">${i}</a>
				</c:forEach>
				<%-- 오른쪽 꺽쇄 '>'  --%>
				<c:if test="${endPage < pageCount}">
					<a href="/petBill/admin/ajaxSearch.pet?pageNum=${startPage + pageBlock}&sidoSel=${sidoSel}&siguSel=${siguSel}" class="page-link"> &gt; </a>
