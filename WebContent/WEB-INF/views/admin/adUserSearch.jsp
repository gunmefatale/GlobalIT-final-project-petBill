...
<head>
  ...
  <!-- ================================================== 공통 메뉴 구분선 ========================================================== -->              
           ...
           <!-- 페이지 내용작성 영역 : Page content-->
			...
			<div align="center">
			<%-- 작성자/내용 검색 --%>
			<form action="/petBill/admin/adUserSearch.pet">
				<select name="sel">
			...
			<table class="table table-bordered">
				...
				<tr>
					...
					<c:if test="${sel == null || search == null}"><!-- 검색안했을때 -->
						<a href="/petBill/admin/adUserContent.pet?pageNum=${pageNum}&id=${article.id}" class="nav-link">${article.id}</a>
					</c:if>			
					<c:if test="${sel != null || search != null}"><!-- 검색했을때 -->
						<a href="/petBill/admin/adUserContent.pet?pageNum=${pageNum}&id=${article.id}&sel=${sel}&search=${search}" class="nav-link">${article.id}</a>
					</c:if>			
					</td>
					<td>
						${article.nickname}
					...
					<td align="center">
						<fmt:formatDate value="${article.reg}" pattern="yy-MM-dd"/>
					</td>
					<td align="center">
						${article.useractivenum}
					</td>
				</tr>
				</c:forEach>
				</table>
		         ...
		         <c:if test="${count > 0}">
					...
					<c:if test="${sel == null || search == null}">
			            <c:if test="${startPage > pageBlock}">
			               <a href="/petBill/admin/adUserSearch.pet?pageNum=${startPage-pageBlock}" class="page-link"> &lt;</a>
			            </c:if>
			            <c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
			               <a href="/petBill/admin/adUserSearch.pet?pageNum=${i}" class="page-link">${i}</a>
			            </c:forEach>
			            <c:if test="${endPage < pageCount}">
			               <a href="/petBill/admin/adUserSearch.pet?pageNum=${startPage+pageBlock}" class="page-link"> &gt; </a>
			            </c:if>
		            ...
		            <c:if test="${sel != null && search != null}">
			            <c:if test="${startPage > pageBlock}">
			               <a href="/petBill/admin/adUserSearch.pet?pageNum=${startPage-pageBlock}&sel=${sel}&search=${search}" class="page-link"> &lt; </a>
			            </c:if>
			            <c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
			               <a href="/petBill/admin/adUserSearch.pet?pageNum=${i}&sel=${sel}&search=${search}" class="page-link">${i}</a>
			            </c:forEach>
			            <c:if test="${endPage < pageCount}">
			               <a href="/petBill/admin/adUserSearch.pet?pageNum=${startPage+pageBlock}&sel=${sel}&search=${search}" class="page-link"> &gt; </a>
			            </c:if>
