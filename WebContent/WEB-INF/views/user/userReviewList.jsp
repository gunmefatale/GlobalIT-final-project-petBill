           ...
           <div align="center">
				...
				<table class="table table-bordered table-hover">
					<caption>마이 리뷰</caption>
						...
						<tr>
							...
							<td align="center">
								<fmt:formatDate value="${article.reviewDate}" pattern="yyyy-MM-dd"/>
							</td>
						</tr>
					</c:forEach>					
				</table>
			...
			<c:if test="${count > 0}">
				...
				<c:if test="${startPage > pageBlock}">
					<a href="/petBill/user/userReviewList.pet?pageNum=${startPage-pageBlock}" class="page-link"> &lt;</a>
				</c:if>
				<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
					<a href="/petBill/user/userReviewList.pet?pageNum=${i}" class="page-link">${i}</a>
				</c:forEach>
				<c:if test="${endPage < pageCount}">
					<a href="/petBill/user/userReviewList.pet?pageNum=${startPage+pageBlock}" class="page-link"> &gt; </a>
				</c:if>
